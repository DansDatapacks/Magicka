...
#if hit block
execute unless block ~ ~ ~ #sourcecraft.2:passable run scoreboard players set #raycast_hit sourcecraft.temp 1
...
#if hit found: end
execute if score #raycast_hit sourcecraft.temp matches 1 run function sourcecraft.2:raycast/simple/end

#else: continue
execute if score #raycast_hit sourcecraft.temp matches 0 unless score #raycast_distance_traveled sourcecraft.temp = #raycast_distance sourcecraft.temp positioned ^ ^ ^0.1 run function sourcecraft.2:raycast/simple/run